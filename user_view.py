...
def home():
    ...
    upcoming_event = Event.query.filter(Event.event_start >= current_datetime).all()
    event_images = Event.query.filter(Event.event_img != None).all()

    return render_template('Homepage.html', categories_nav=categories_nav, events=upcoming_event, event_images=event_images, user=current_user)
    ...
    session = stripe.checkout.Session.create(
        ...
        line_items=[{
            'price_data': {
                ...
                'unit_amount': int(float(ticket_price) * 100),
            },
            'quantity': 1,
        }],
        mode='payment',
...
def manage_membership(admin_id, action):
    ...
    return redirect(url_for('user_view.organisers'))
